...
# ==========================
# VIDEO STREAM OTTIMIZZATO
# ==========================
frame_buffer = None

def start_video_stream():
    """Avvia lo streaming video (thread per acquisizione, GUI aggiorna via after)."""
    ...
    if not streaming:
        try:
            ...
            # Thread che legge i frame dal Tello
            frame_thread = threading.Thread(target=frame_capture_loop, daemon=True)
            frame_thread.start()

            # Avvia aggiornamento GUI nel main thread
            update_video_gui()
...
def frame_capture_loop():
    """Cattura continuamente i frame del Tello in un buffer globale."""
    global frame_buffer, streaming
    frame_read = tello.get_frame_read()

    while streaming:
        frame_buffer = frame_read.frame
        time.sleep(0.01)  # evita consumi CPU eccessivi


def update_video_gui():
    """Aggiorna i frame nel MAIN thread usando after(), evitando scatti."""
    global frame_buffer

    if frame_buffer is not None:
        img = frame_buffer

        # Riduci dimensioni per fluidità
        img = cv2.resize(img, (320, 240))

        # Converti in RGB e poi in Tkinter image
